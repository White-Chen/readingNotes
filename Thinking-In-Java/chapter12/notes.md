...
+ [对于在构造阶段可能会抛出的异常，并且要求清理的类，**最安全**的使用方式是使用嵌套的try子句]()  
    >比如在构造阶段，如果需要打开一个文件流，这里显然需要进行异常捕获，如果发生异常表示构造失败此时需要关闭文件流，但是如果没有发生异常那么文件流就不应该关闭而是应该被其他方法调用，此时把 _close()_ 方法放在finally块中显然是不对的，这回导致正常的构造变得不正常。这时就应该在try中区根据情况关闭文件流，但是在关闭时也是会报错的，这时应该用嵌套try。
