...
+ _**garbage collector (GC)**_ 并不是析构函数, 因此只能释放对象占用的内存空间, 而当对象被标记时将被释放前, 你无法定义一些内存被释放前必须执行的行为. 
...
+ _**finalize()**_ 正确的用途是用于确认终止状态, 虽然不能人为控制 _**finalize()**_ 被调用的时间, 但是可以保证 _**finalize()**_ 被调用那么对象就意味着将被释放, 这就意味对象进入终止状态, 此时好的编程习惯是进行一些状态确认, 确保对象被释放前进行了正确的操作, 如果操作不正确应该通过日志等方式进行记录以便后续粉分析. :bangbang:
